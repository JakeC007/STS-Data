...
df_f3 <- df_f3[, c(1,3,2,4)] #change order of cols for facets to come in nice order 
...
## Center No Percents 
plot(likert(df_f3.reverse2[,c(1:3)], grouping = df_f3.reverse[,4]), legend.position="right", centered = TRUE, plot.percents=TRUE, plot.percent.low=FALSE, plot.percent.high=FALSE, plot.percent.neutral=FALSE, center = 2.5) + 
  scale_fill_manual(values = brewer.pal(n=4,"RdBu"), breaks = c("Not at all", "Not too much", "Somewhat", "A great deal")) + #order and color the likert boxes
  ggtitle(title2) +
  guides(fill = guide_legend(title="Responses", reverse = TRUE)) #to reverse the order in the legend
